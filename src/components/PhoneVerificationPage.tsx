...
import { DemoPhoneVerification } from './DemoPhoneVerification';
...
const PhoneVerificationPage: React.FC<PhoneVerificationPageProps> = ({
  ...
  // デモアクセス処理
  const handleDemoAccess = (token: string) => {
    setDemoToken(token);
    setIsDemoMode(true);
  };

  // デモ認証成功処理
  const handleDemoVerificationSuccess = (sessionData: any) => {
    // デモセッションデータをセッションストレージに保存
    sessionStorage.setItem('currentUserSession', JSON.stringify({
      ...userSession,
      ...sessionData,
      isDemo: true
    }));
  ...
  // デモモードの場合は専用コンポーネントを表示
  if (isDemoMode && demoToken) {
    return (
      <DemoPhoneVerification
        demoToken={demoToken}
            ...
            <form onSubmit={handleSendOTP} className="space-y-6">
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-yellow-800 font-semibold text-sm mb-1">デモ環境を試す</h4>
